...
const Game: React.FC = () => {
  ...
  const [userAnswer, setUserAnswer] = useState<number | null>(null);
  const [answerFeedback, setAnswerFeedback] = useState<string | null>(null);
  const [score, setScore] = useState(currentUser?.tester[0]?.score || 0);  // Use first tester's score
  const router = useRouter();
  ...
  const handleTimerEnd = () => {
    ...
    if (userAnswer !== null) {
      if (userAnswer === correct) {
        ...
        setScore(newScore);
        if (currentUser) {
          updateTestTakerScore(currentUser.tester[0]?.id || -1, newScore);  // Update score for first tester
        }
      } else {
        setAnswerFeedback('Incorrect!');
      }
        ...
        <ul>
          {currentQuestion?.answers.map((answer, index) => (
            <li
              key={index}
              onClick={() => handleAnswerClick(index)}
              className={`p-2 border cursor-pointer ${userAnswer === index
                ? timer === 0
                  ? index === currentQuestion.correctAnswer
                    ? 'border-green-500 bg-green-100'
                    : 'border-red-500 bg-red-100'
