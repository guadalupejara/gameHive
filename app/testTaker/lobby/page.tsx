  ...
  useEffect(() => {
    ...
    const fetchTesters = async () => {
      ...
      if (quiz?.id) {
        ...
        console.error('quiz id was undefined');
      }
    };
    ...
    const unsubscribe = listenToTestTakers(quiz?.id, (updatedTesters: TestTaker[]) => {
      // When test takers are updated, set the testTakers context
    });

    return () => {
      unsubscribe();
    };
  }, [gameStarted, router, quiz?.id]);

  // Flatten the tester array from testTakers and pass it to LobbyBox
  const allTesters = testTakers.flatMap((testTaker) => testTaker.tester);

  return (
    <main className="flex min-h-screen flex-col items-center justify-center">
      <h1 className="text-4xl font-bold mb-8">Lobby</h1>
      <p className="text-lg mb-4">Waiting for the Test Maker to start the game...</p>
      <LobbyBox takers={allTesters} />
    </main>
  );
};
